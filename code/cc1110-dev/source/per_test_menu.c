...
void checkChipVersion(void)
{
    ...
    if (((PARTNUM == PARTNUM_CC1110) && VERSION < CC1110_MIN_SUPPORTED_VERSION) ||
        ...
        while (1) {
            selectFromMenu((const char **) versionOldMenu, VERSION_OLD_MENU_LINES, 0);
        }
        ...
        while (1) {
            selectFromMenu((const char **) versionNewMenu, VERSION_NEW_MENU_LINES, 0);
        }
    }
...
void lcdWritePer(void)
{
    ...
    UINT32 per = (((perExpectedSeqNum - 1) - perRcvdPkts + perBadPkts)*1000)/(perExpectedSeqNum - 1);
